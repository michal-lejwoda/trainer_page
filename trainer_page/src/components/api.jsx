...
export async function getTrainers() {
    const response = await instance.get(`${DOMAIN}/api/trainers`);
...
export async function getTrainerPlan(data) {
    const response = await instance.post(`${DOMAIN}/api/get_trainer_plans`, data);
    let list_of_elements = []
    for (let i = 0; i < response.data.length; i++) {
        list_of_elements.push({
...
export async function getDayWorkHours(data) {
    const response = await instance.post(`${DOMAIN}/api/get_day_work_hours`, data)
...
export async function sendConfirmationEmail(data){
    const response = await instance.post(`${DOMAIN}/api/send-email/background_task`, data, {withCredentials: true})
    return response.data
}